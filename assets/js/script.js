...
for (let i = 0; i < testimonialsItem.length; i++) {
  testimonialsItem[i].addEventListener('click', function () {
    ...
    modalDate.datetime = this.querySelector('[data-testimonials-date]').datetime;
...
const fetchArticles = () => {
  ...
  const raw = JSON.stringify({
    query:
      'query Publication{\n    publication(host: "kenny82.hashnode.dev"){\n      author{\n        username\n      },\n    posts(first:10){\n      edges{\n        node{\n          id,\n          title,\n          slug, \n          readTimeInMinutes,\n          brief,\n          coverImage{\n            url\n          }\n        }\n      }\n    }\n     \n    }\n}\n',
  ...
  fetch('https://gql.hashnode.com', requestOptions)
    ...
    .then((result) => {
      ...
      displayArticles(articles.filter((_, index) => index !== 1)); // Call function to display articles
    })
...
const displayArticles = (articles) => {
  ...
  articles.forEach((article) => {
    ...
    listItem.innerHTML = `
      <a href="https://kenny82.hashnode.dev/${article.slug}" target="_blank" rel="noopener">

        <figure class="blog-banner-box">
          <img src="${article.coverImage?.url || './assets/images/default-image.jpg'}" alt="${
      article.title
    }" loading="lazy">
        </figure>
